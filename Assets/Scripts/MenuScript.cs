...
public class MenuScript : MonoBehaviour {
	...
	void Start ()
	{
		...
		menuPanel = transform.Find("PengaturanPanel");
		...
		{
			if(menuPanel.GetChild(i).name == "btnrun")
				menuPanel.GetChild(i).GetComponentInChildren<Text>().text = GameManager.GM.forward.ToString();
			else if(menuPanel.GetChild(i).name == "btnbackward")
				menuPanel.GetChild(i).GetComponentInChildren<Text>().text = GameManager.GM.backward.ToString();
			else if(menuPanel.GetChild(i).name == "btnleft")
				menuPanel.GetChild(i).GetComponentInChildren<Text>().text = GameManager.GM.left.ToString();
			else if(menuPanel.GetChild(i).name == "btnright")
				menuPanel.GetChild(i).GetComponentInChildren<Text>().text = GameManager.GM.right.ToString();
			else if(menuPanel.GetChild(i).name == "btnjump")
				menuPanel.GetChild(i).GetComponentInChildren<Text>().text = GameManager.GM.jump.ToString();
			else if(menuPanel.GetChild(i).name == "btnak47")
				menuPanel.GetChild(i).GetComponentInChildren<Text>().text = GameManager.GM.ak47.ToString();
			else if(menuPanel.GetChild(i).name == "btnshotgun")
				menuPanel.GetChild(i).GetComponentInChildren<Text>().text = GameManager.GM.shotgun.ToString();
	...
	public IEnumerator AssignKey(string keyName)
	{
		...
		case "jump":
			...
			break;
		case "ak47":
			GameManager.GM.ak47 = newKey; //set jump to new keycode
			buttonText.text = GameManager.GM.ak47.ToString(); //set button text to new key
			PlayerPrefs.SetString("Alpha1", GameManager.GM.ak47.ToString()); //save new key to playerprefs
			break;
		case "shotgun":
			GameManager.GM.shotgun = newKey; //set jump to new keycode
			buttonText.text = GameManager.GM.shotgun.ToString(); //set button text to new key
			PlayerPrefs.SetString("Alpha2", GameManager.GM.shotgun.ToString()); //save new key to playerprefs
			break;
